...
namespace Maps
{
    public class NewMap
    {
       ...
       //отрисовка полигона на карте
        ...
        public void SaveTubeToDB(string id_expert, string descriptionText, string name, string idOfIssue, string calNum, string idOfFormula)
        {
            int maXNum;
            ...
            if (_points.Count >= 2)
            {
                ...
                "line_alfa", "line_thickness", "name","id_of_expert", "description", "type"};
                string[] val = { Convert.ToString(maXNum), "0", "0", "0", "250", "0", "250", "2", "21", "2", $"'{name}'", id_expert, $"'{descriptionText}'", $"'tube'" };
        ...
        //сохранения полигона
        public void SaveCoordinateToDataBase(string id_issue, string id_calc, string id_formula, string id_expert, ColorDialog colorDialog, string descriptionText, string name)
        {
            int maXNum;
            //get the max poligon id0
            var res_points = db.GetRows("poligon", "max(Id_of_poligon)", "");
            ...
            string[] fields = { "Id_of_poligon", "brush_color_r", "bruch_color_g", "brush_color_b", "brush_alfa", "line_collor_r", "line_color_g", "line_color_b",
                "line_alfa", "line_thickness", "name","id_of_expert", "description", "type"};
            string[] val = { Convert.ToString(maXNum), colorDialog.Color.R.ToString(), colorDialog.Color.G.ToString(), colorDialog.Color.B.ToString(), "250", "0", "250", "2", "21", "2", $"'{name}'", id_expert, $"'{descriptionText}'", $"'poligon'" };
            db.InsertToBD("poligon", fields, val);
            //points
            string[] fields_1 = { "longitude", "latitude", "Id_of_poligon", "order123" };
            int count = 1;
        ...
        public void LoadImageForMarkets()
        {

        }
        //Добавить в БД объект
        public void AddObjectToDB(string name, int type, double lat, double lng, string descriptions, string NameObject, string idOfIssue, string calNumber, string idOfFormula)
        {
            string[] field = { "Name", "Type", "Coord_Lat", "Coord_Lng", "Description", "Name_Object", "id_of_issue", "calculations_description_number", "id_of_formula"};
            string[] values = { "'"+name+"'", type.ToString(), lat.ToString().Replace(',', '.'), lng.ToString().Replace(',', '.'), "'"+descriptions+"'", "'" + NameObject + "'",
        ...
        public void AddObjectFromDB()
        {
            ...
            gMapControl.Show();
            list = db.GetRows("poi", "Coord_Lat, Coord_Lng, Description, Type, Name, Name_Object", "");
        ...
        public void AddObjectFromDB(string condition)
        {
            ...
            gMapControl.Overlays.Clear();
            gMapControl.Show();
            list = db.GetRows("poi", "Coord_Lat, Coord_Lng, Description, Type, Name_Object", "Name = " + condition);
            for (int i = 0; i < list.Count; i++)
            {
                System.IO.MemoryStream image_stream = new System.IO.MemoryStream((byte[])db.GetValue("type_of_object", "Image", "Id = " + list[i][3]));
                Image img = Image.FromStream(image_stream);
