...
namespace Experts_Economist
{
    public partial class Redakt : Form
    {
        ...
        private void get_values()
        {
            ...
            var obj2 = db.GetRows("formulas", "name_of_formula", "id_of_expert = " + id_of_exp);
            item = "";
            ...
            var obj3 = db.GetRows("formulas", "id_of_formula", "id_of_expert = " + id_of_exp);
            item = "";
        ...
        //при добавлении формулы должен обновлятся и список параметров, поскольку каждая формула
        //является и параметром одновременно для других формул
        private void add1_Click(object sender, EventArgs e)
        {//переменные для хранения имён формул, описания и id експерта
            if (id_of_exp == 0)
                id_of_exp = 1;
            var count = db.GetValue("formulas","COUNT(1)",$"name_of_formula  = \"{formula_nameTB.Text.Trim()}\" and id_of_expert = {id_of_exp} ;");
            DialogResult dialogResult=DialogResult.None;
            if (Convert.ToInt32(count) > 0)
            {
                var matched = db.GetRows("formulas", "name_of_formula, description_of_formula", $"name_of_formula = \"{formula_nameTB.Text.Trim()}\" and id_of_expert = {id_of_exp}");
                dialogResult = MessageBox.Show($"Така формула вже існує!\nНазва: {matched[0][0]}\nОпис: {matched[0][1]}\nЗамінити її на поточну?","Увага!",MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            }
            ...
            // MessageBox.Show();
            refresh();//обновляет все листбоксы новыми значениями
        ...
        private void componentLB_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if ((nIdx >= 0) && (nIdx < componentLB.Items.Count))
            {
                ...
                var obj = db.GetRows("formulas", "description_of_formula", "id_of_formula = " + idfp + " AND id_of_expert = " + id_of_exp);
                strTip = obj[0][0].ToString();
            }
        ...
        private void formulasLB_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    obj1 = db.GetRows("formulas", "name_of_formula", "id_of_formula = " + obj[i][0].ToString() + " AND id_of_expert = " + id_of_exp);
        ...
        private void componentLB_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                var obj = db.GetRows("formulas", "measurement_of_parameter,description_of_parameter ", "id_of_parameter =" + idfp + " AND id_of_expert = " + id_of_exp);
